...
#include "WifiConfig.h"   // g_wifi -- runtime WiFi creds, set via the "wifi" command
#endif
...
// MeshCore well-known PUBLIC channel key (base64 "izOH6cXN6mrJ5e26oRXNcg==").
// This is public by design -- holding it lets the repeater decrypt public
// group-text packets it relays so the HTTP page can show recent chatter. DMs
// and private channels stay opaque (we hold no keys for them).
static const uint8_t PUBLIC_CHANNEL_PSK[16] = {
  0x8B, 0x33, 0x87, 0xE9, 0xC5, 0xCD, 0xEA, 0x6A,
  0xC9, 0xE5, 0xED, 0xBA, 0xA1, 0x15, 0xCD, 0x72
};

void MyMesh::beginPublicChannel() {
  memset(_pub_chan.secret, 0, sizeof(_pub_chan.secret));
  memcpy(_pub_chan.secret, PUBLIC_CHANNEL_PSK, sizeof(PUBLIC_CHANNEL_PSK));
  // Channel hash is sha256(secret, 16) (128-bit key), matched on the first byte.
  mesh::Utils::sha256(_pub_chan.hash, sizeof(_pub_chan.hash),
                      _pub_chan.secret, sizeof(PUBLIC_CHANNEL_PSK));
  _pub_chan_ready = true;
  memset(_chan_count, 0, sizeof(_chan_count));
  memset(_chan_last, 0, sizeof(_chan_last));
}

// Called for every unseen group-text packet. Tally activity per channel hash
// (this is our window into channels we can't decrypt), then offer the public
// channel key so the base Mesh can decrypt public messages.
int MyMesh::searchChannelsByHash(const uint8_t* hash, mesh::GroupChannel channels[], int max_matches) {
  _chan_count[hash[0]]++;
  _chan_last[hash[0]] = millis() / 1000;
  if (max_matches > 0 && _pub_chan_ready && hash[0] == _pub_chan.hash[0]) {
    channels[0] = _pub_chan;
    return 1;
  }
  return 0;
}

// Called by the base Mesh after a public group packet decrypts. Parse the
// "name: message" text (mirrors BaseChatMesh) and keep it in the ring buffer.
void MyMesh::onGroupDataRecv(mesh::Packet* packet, uint8_t type, const mesh::GroupChannel& channel, uint8_t* data, size_t len) {
  ...
  if ((txt_type >> 2) != 0) return;          // plain text only
  uint32_t timestamp; memcpy(&timestamp, data, 4);
  data[len] = 0;                             // C-string terminate the decrypted text

  PublicChatMsg& m = _chatlog[_chatlog_head];
  m.rx_uptime     = millis() / 1000;
  m.msg_timestamp = timestamp;
  m.snr           = packet ? (int8_t)(packet->getSNR() * 4) : 0;
  strncpy(m.text, (const char*)&data[5], sizeof(m.text) - 1);
  m.text[sizeof(m.text) - 1] = 0;

  _chatlog_head = (_chatlog_head + 1) % PUBLIC_CHAT_LOG;
  if (_chatlog_count < PUBLIC_CHAT_LOG) _chatlog_count++;
  _public_msg_total++;
}

// Upsert a node we heard an advert from, keyed by pubkey prefix. When the table
// is full, evict the least-recently-heard entry.
void MyMesh::recordHeardNode(const mesh::Identity& id, const char* name, uint8_t type,
                             int8_t snr, uint8_t hops) {
  int slot = -1;
  for (int i = 0; i < _heard_count; i++) {
    if (memcmp(_heard[i].pubkey, id.pub_key, sizeof(_heard[i].pubkey)) == 0) { slot = i; break; }
  }
  if (slot < 0) {
    if (_heard_count < HEARD_NODES) {
      slot = _heard_count++;
    } else {
      uint32_t oldest = 0xFFFFFFFF;
      slot = 0;
      for (int i = 0; i < HEARD_NODES; i++)
        if (_heard[i].last_uptime < oldest) { oldest = _heard[i].last_uptime; slot = i; }
    }
    memset(&_heard[slot], 0, sizeof(_heard[slot]));
    memcpy(_heard[slot].pubkey, id.pub_key, sizeof(_heard[slot].pubkey));
  }
  HeardNode& h = _heard[slot];
  if (name && name[0]) { strncpy(h.name, name, sizeof(h.name) - 1); h.name[sizeof(h.name) - 1] = 0; }
  h.type = type;
  h.snr = snr;
  h.hops = hops;
  h.last_uptime = millis() / 1000;
  h.count++;
}
#endif // WITH_WIFI
...
#if defined(WITH_WIFI)
  // Record every advertising node we hear (any type, direct or relayed) with its
  // advertised name, for the HTTP "Nodes heard" view.
  if (parser.isValid()) {
    recordHeardNode(id, parser.hasName() ? parser.getName() : "",
                    parser.getType(), (int8_t)(packet->getSNR() * 4),
...
void MyMesh::begin(FILESYSTEM *fs) {
  ...
  _cli.loadPrefs(_fs);
#if defined(WITH_WIFI)
  beginPublicChannel();   // hold the public channel key for the HTTP message view
#endif
...
#if defined(ESP32) && defined(WITH_WIFI)
  // Runtime WiFi config: "wifi", "wifi ssid <x>", "wifi pass <x>", "wifi on|off",
  // "wifi reconnect". Handled here so it works identically over serial and the
  // TCP console (both route through handleCommand). Creds persist on the node.
  if (memcmp(command, "wifi", 4) == 0 && (command[4] == 0 || command[4] == ' ')) {
    if (g_wifi) g_wifi->handleCommand(command + 4, reply, 150, _fs);
    else        strcpy(reply, "Err - wifi unavailable");
    return;
  }
#endif
