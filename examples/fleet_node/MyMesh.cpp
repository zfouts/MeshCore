...
void MyMesh::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp,
  ...
  handleBotChannel(channel, pkt, timestamp, text); // control room answers on the control channel
#endif
  int i = 0;
  if (app_target_ver >= 3) {
...
MyMesh::MyMesh(mesh::Radio &radio, mesh::RNG &rng, mesh::RTCClock &rtc, SimpleMeshTables &tables, DataStore& store, AbstractUITask* ui)
  ...
  _prefs.client_repeat = 1;     // fleet_node: act as a relay by default (first boot)
#endif
#ifdef WITH_FLEET_EXTRAS
  _prefs.bot_enabled = 1;       // control room answers by default
  _prefs.ble_enabled = 1;       // BLE advertising on by default (toggle via `@name ble off`)
  _prefs.bot_control_channel = 0xFF; // control channel off until configured
#endif
#if defined(WITH_FLEET_EXTRAS) && (ENV_INCLUDE_GPS == 1)
  _prefs.gps_enabled = 1;                       // mobile node: track position from GPS
  _prefs.gps_interval = FLEET_GPS_INTERVAL_S; // (first-boot defaults; saved prefs win)
...
#ifdef WITH_FLEET_EXTRAS
  fleetBegin(); // watchdog, persisted config, stats
#endif
}
...
void MyMesh::startInterface(BaseSerialInterface &serial) {
  _serial = &serial;
#if defined(WITH_FLEET_EXTRAS) && defined(BLE_PIN_CODE)
...
#ifdef WITH_FLEET_EXTRAS
    dp = fleetAppendVars((char *)&out_frame[1], dp, (const char *)&out_frame[MAX_FRAME_SIZE]); // expose bot_enable / bot_control_channel
#endif
...
#ifdef WITH_FLEET_EXTRAS
      if (fleetSetVar(sp, np)) { writeOKFrame(); } else
#endif
      {
...
#endif
#ifdef WITH_FLEET_EXTRAS
  fleetLoop(); // watchdog feed, low-batt shutdown, periodic mobile advert
#endif
}
...
// Self-advert masquerading as a chosen adv_type (for `@name advert
// repeater|companion`). Same shape as BaseChatMesh::createSelfAdvert but with a
// caller-supplied type; honours advert_loc_policy. self_id / createAdvert are
// protected members of Mesh, reachable here.
mesh::Packet* MyMesh::createAdvertAs(uint8_t adv_type) {
  uint8_t app_data[MAX_ADVERT_DATA_SIZE];
  uint8_t len;
  if (_prefs.advert_loc_policy == ADVERT_LOC_NONE) {
    AdvertDataBuilder b(adv_type, _prefs.node_name);
    len = b.encodeTo(app_data);
  } else {
    AdvertDataBuilder b(adv_type, _prefs.node_name, sensors.node_lat, sensors.node_lon);
    len = b.encodeTo(app_data);
  }
