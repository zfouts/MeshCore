...
void DataStore::loadPrefsInt(const char *filename, NodePrefs& _prefs, double& node_lat, double& node_lon) {
  ...
  if (file) {
    ...
    file.read((uint8_t *)&_prefs.bot_enabled, sizeof(_prefs.bot_enabled));                 // 137 (fleet_node; absent in old files -> default kept)
    file.read((uint8_t *)&_prefs.ble_enabled, sizeof(_prefs.ble_enabled));                 // 138 (fleet_node; absent in old files -> default kept)
    file.read((uint8_t *)&_prefs.bot_control_channel, sizeof(_prefs.bot_control_channel)); // 139 (fleet_node; absent in old files -> default kept)
    file.read((uint8_t *)&_prefs.boot_count, sizeof(_prefs.boot_count));                   // 140 (fleet_node; absent in old files -> default kept)
...
void DataStore::savePrefs(const NodePrefs& _prefs, double node_lat, double node_lon) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs.bot_enabled, sizeof(_prefs.bot_enabled));                 // 137 (fleet_node)
    file.write((uint8_t *)&_prefs.ble_enabled, sizeof(_prefs.ble_enabled));                 // 138 (fleet_node)
    file.write((uint8_t *)&_prefs.bot_control_channel, sizeof(_prefs.bot_control_channel)); // 139 (fleet_node)
    file.write((uint8_t *)&_prefs.boot_count, sizeof(_prefs.boot_count));                   // 140 (fleet_node)
