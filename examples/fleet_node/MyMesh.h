...
// fleet_node extension (examples/fleet_node). Inert in all other envs.
...
// fleet_node repeater-grade forward policy (examples/fleet_node/RelayPolicy.cpp).
...
// rest of the fleet_node tunables are defined in FleetNode.h. Keeping the
// #ifndef means FleetNode.h shares this value rather than clashing.
#ifndef FLEET_GPS_INTERVAL_S
#define FLEET_GPS_INTERVAL_S 120
#endif
#endif
...
private:
...
#ifdef WITH_RELAY_POLICY
  // Implemented in examples/fleet_node/RelayPolicy.cpp. Repeater-grade
  // forward filter (hop limits + loop detection); inert in other envs.
  bool relayPolicyAllows(const mesh::Packet* packet);
#endif
#ifdef WITH_FLEET_EXTRAS
  // Robustness extensions implemented in examples/fleet_node/FleetNode.cpp:
  ...
  struct FleetState* _fleet = nullptr;
  void fleetBegin();
  void fleetLoop();
  void fleetLowBattBeacon(uint16_t mv);
  int fleetFormatSensorsText(char* out, size_t sz);  // compact `@name sensors` summary
  int fleetResolveChannelArg(const char* value);  // channel idx, 0xFF=off, -1=fail
  int fleetFormatChannelVar(char* buf, int n, size_t bufsz, const char* name, uint8_t ch);
  bool fleetSetVar(const char* name, const char* value);
  char* fleetAppendVars(char* base, char* dp, const char* end);
  void handleBotChannel(const mesh::GroupChannel& channel, mesh::Packet* pkt, uint32_t timestamp, const char* text);
  void handleTargetedCmd(const mesh::GroupChannel& channel, const char* sender, const char* text);
  // Self-advert masquerading as a chosen adv_type (repeater/companion) for
  // `@name advert`. Mirrors BaseChatMesh::createSelfAdvert but with a caller-
  // supplied type; honours advert_loc_policy. Defined in MyMesh.cpp.
  mesh::Packet* createAdvertAs(uint8_t adv_type);
#endif
