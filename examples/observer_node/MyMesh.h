...
#define FIRMWARE_VERSION "v1.16.0-observer"
#endif
...
// rest of the observer_node tunables are defined in ObserverNode.h. Keeping the
// #ifndef means ObserverNode.h shares this value rather than clashing.
#ifndef OBS_GPS_INTERVAL_S
#define OBS_GPS_INTERVAL_S 120
#endif
#endif
...
struct AdvertPath {
  ...
  int8_t  snr_x4;               // SNR of the advert as heard by THIS node, x4
...
public:
  ...
  bool sendMqttChannelText(const char* channel, const char* text);
...
#ifdef WITH_OBSERVER_EXTRAS
  // JSON payloads for the MQTT publisher (main.cpp, _wifi builds); public
  // because the publisher lives outside the class.
  int observerFormatMqttTelemetry(char* buf, size_t sz);
  int observerFormatSensorsJson(char* out, size_t sz);   // all attached sensor readings
#endif
...
private:
...
#endif
#ifdef WITH_OBSERVER_PROBE
  // Active telemetry prober (examples/observer_node/ObserverProbeGlue.cpp).
  // Only in *_observer_node_* envs. Walks the repeater roster and, twice a day
  // per repeater, drives a blank-password login + GET_TELEMETRY_DATA request,
  // reusing the same sendLogin()/sendRequest()/onContactResponse() path the
  // phone app uses. Single probe in flight; state advanced from the response
  // hooks. See ObserverProbe.h for the schedule/backoff policy.
  ObserverProbe _probe;
  enum { PROBE_IDLE, PROBE_WAIT_LOGIN, PROBE_WAIT_TELEM };
  uint8_t  _probe_state = PROBE_IDLE;
  uint8_t  _probe_key[4] = {0, 0, 0, 0};
  uint32_t _probe_deadline = 0;    // millis; in-flight login/telemetry timeout
  uint32_t _probe_next_scan = 0;   // millis; min gap between probe attempts
  int      _probe_cursor = 0;      // round-robin position in the contact table
  void observerProbeLoop();
  void observerProbeOnLogin(const ContactInfo& contact, bool ok);
  void observerProbeOnTelemetry(const ContactInfo& contact, const uint8_t* lpp, uint8_t lpp_len);
  void observerPublishPath(const ContactInfo& contact);
#endif
#ifdef WITH_OBSERVER_EXTRAS
  // Robustness extensions implemented in examples/observer_node/ObserverNode.cpp:
  ...
  struct ObserverState* _observer = nullptr;
  void observerBegin();
  void observerLoop();
  void observerOnRx(float snr, float rssi);
  void observerOnNeighbour(const ContactInfo& contact, uint8_t path_len);
  void observerCountForward(bool allowed);
  void observerLowBattBeacon(uint16_t mv);
  void observerFormatStats(char* reply, size_t sz);
  void observerFormatNeighbours(char* reply, size_t sz);
  void observerFormatHeard(const char* arg, char* reply, size_t sz);  // `!heard <name|hex>`
  ...
  // SNR of the advert packet currently being processed. onDiscoveredContact()
  // isn't handed the packet, but BaseChatMesh::onAdvertRecv() calls it
  // synchronously, so the onAdvertRecv override stashes the value here for the
  // heard-table entry. (RSSI is deliberately absent: Packet doesn't carry it,
  // and by inbound-queue time the radio's last-RSSI can be another packet's.)
