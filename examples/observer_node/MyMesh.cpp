...
#ifdef WITH_OBSERVER_EXTRAS
  observerOnNeighbour(contact, path_len); // track directly-heard neighbours
#endif
  if (_serial->isConnected()) {
    if (is_new) {
...
// MQTT->mesh send bridge (called from the main loop; main.cpp's subscriber
// queues into it from the esp_mqtt task). Deliberately narrow: an MQTT client
// can post text INTO A CHANNEL THIS NODE ALREADY HOLDS KEYS FOR, as this node,
// and nothing else -- no contact DMs, no CLI/admin path. Same send as the
// companion's CMD_SEND_CHANNEL_TXT_MSG, minus the serial frame.
//
// `channel` is a decimal slot index ("0".."255") OR a channel name. Names are
// matched case-insensitively and a leading '#' on the STORED name is ignored
// (MQTT publish topics can't contain '#'), so "send/bot" reaches "#bot".
// Name addressing is what keeps a fleet consistent: slot order is per-node
// config history, the name travels with the channel.
bool MyMesh::sendMqttChannelText(const char* channel, const char* text) {
  if (channel[0] == 0) return false;
  ...
  int idx = -1;
  if (numeric) {
    if (strlen(channel) > 3) return false;
    idx = atoi(channel);
    ...
    for (int i = 0; i < MAX_GROUP_CHANNELS; i++) {
      ...
      const char* stored = (cd.name[0] == '#') ? &cd.name[1] : cd.name;
      if (strcasecmp(stored, channel) == 0) { idx = i; break; }
    }
    if (idx < 0) return false;   // no such channel on this node -> send nothing
  }

  ChannelDetails dest;
  if (!getChannel(idx, dest)) return false;
  if (dest.name[0] == 0) return false;   // unconfigured channel slot
  int text_len = strlen(text);
  if (text_len == 0) return false;
  if (text_len > MAX_TEXT_LEN) text_len = MAX_TEXT_LEN;
  return sendGroupMessage(getRTCClock()->getCurrentTimeUnique(), dest.channel,
...
void MyMesh::onContactPathUpdated(const ContactInfo &contact) {
  ...
  observerPublishPath(contact);   // hop chain learned by a probe -> map/topology
#endif
...
#ifdef WITH_OBSERVER_EXTRAS
// MQTT message mirror: every readable received message goes to the broker on
// _wifi builds with `set mqtt_host` configured. Strong def in main.cpp; weak
...
static int fmtRxHops(const mesh::Packet* pkt, char* buf, size_t sz) {
  buf[0] = 0;
  if (!pkt || !pkt->isRouteFlood()) return 0;
  int nb = pkt->getPathByteLen();   // count * hash-size; path_len is ENCODED, not a byte count
  if (nb < 0 || nb > MAX_PATH_SIZE) nb = 0;
  int o = 0;
  for (int i = 0; i < nb && o + 2 < (int)sz; i++)
    o += snprintf(buf + o, sz - o, "%02x", pkt->path[i]);
  buf[o] = 0;
  return pkt->getPathHashCount();
}
#endif
...
void MyMesh::onContactResponse(const ContactInfo &contact, const uint8_t *data, uint8_t len) {
  ...
  if (pending_login && memcmp(&pending_login, contact.id.pub_key, 4) == 0) { // check for login response
    ...
    _serial->writeFrame(out_frame, i);
#ifdef WITH_OBSERVER_PROBE
    observerProbeOnLogin(contact, out_frame[0] == PUSH_CODE_LOGIN_SUCCESS);
#endif
    ...
    _serial->writeFrame(out_frame, i);
#ifdef WITH_OBSERVER_PROBE
    observerProbeOnTelemetry(contact, &data[4], len - 4);
#endif
...
#endif
#endif
#if defined(WITH_OBSERVER_EXTRAS) && (ENV_INCLUDE_GPS == 1)
  _prefs.gps_enabled = 1;                       // mobile node: track position from GPS
  _prefs.gps_interval = OBS_GPS_INTERVAL_S; // (first-boot defaults; saved prefs win)
...
#ifdef WITH_OBSERVER_EXTRAS
  observerBegin(); // watchdog, persisted config, stats
#endif
}
...
void MyMesh::startInterface(BaseSerialInterface &serial) {
  _serial = &serial;
#if defined(WITH_OBSERVER_EXTRAS) && defined(BLE_PIN_CODE)
  ...
  if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
...
#ifdef WITH_OBSERVER_EXTRAS
    dp = observerAppendVars((char *)&out_frame[1], dp, (const char *)&out_frame[MAX_FRAME_SIZE]); // expose bot_enable / bot_channel
#endif
    ...
    if (np) {
      *np++ = 0; // modify 'cmd_frame', replace ':' with null
#ifdef WITH_OBSERVER_EXTRAS
      if (observerSetVar(sp, np)) { writeOKFrame(); } else
#endif
      {
      ...
      } // end observer-var fall-through block
    } else {
      writeErrFrame(ERR_CODE_ILLEGAL_ARG);
    }
...
#endif
#ifdef WITH_OBSERVER_EXTRAS
  observerLoop(); // watchdog feed, low-batt shutdown, periodic mobile advert
#endif
#ifdef WITH_OBSERVER_PROBE
  observerProbeLoop(); // scheduled blank-password telemetry probe of repeaters
#endif
}
