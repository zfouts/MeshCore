// observer_node — shared runtime state for the robustness extensions.
//
// Held by MyMesh via a single `ObserverState* _observer` pointer (see the
// WITH_OBSERVER_EXTRAS block in companion_radio/MyMesh.h) so the companion
// header only gains one member. All observer_node .cpp files include this.
//
// Tunables are compile-time build flags (override in the env build_flags); a
// car repeater is set-and-flash, so there is intentionally no over-the-mesh
// admin surface.

#pragma once

#include <stdint.h>
#include "RateLimiter.h"

// ---- build-time tunables (override any of these in the env build_flags) ----
#ifndef OBS_LOW_BATT_MV
#define OBS_LOW_BATT_MV 0               // auto power-off below this (0 = DISABLED).
#endif                                       // Off by default: a car/vehicle repeater
                                             // must never deep-sleep itself. Set >0 only
                                             // for genuinely battery-powered deployments.
#ifndef OBS_LOW_BATT_FLOOR
#define OBS_LOW_BATT_FLOOR 2500         // below this we assume "no battery / USB" and
#endif                                       // never shut down (avoids a floating ADC trip)
#ifndef OBS_GPS_INTERVAL_S
#define OBS_GPS_INTERVAL_S 120          // GPS refresh interval while mobile
#endif
#ifndef OBS_ADVERT_INTERVAL_S
#define OBS_ADVERT_INTERVAL_S 21600     // periodic zero-hop location advert (0 = off)
#endif
#ifndef OBS_WDT_TIMEOUT_S
#define OBS_WDT_TIMEOUT_S 30            // hardware watchdog timeout (ESP32)
#endif
#ifndef OBS_BOT_RATE_MAX
#define OBS_BOT_RATE_MAX 6              // max bot replies ...
#endif
#ifndef OBS_BOT_RATE_SECS
#define OBS_BOT_RATE_SECS 60            // ... per this many seconds
#endif
#ifndef OBS_WD_INTERVAL_S
#define OBS_WD_INTERVAL_S 45            // wardrive-mode beacon interval (`!wd on`)
#endif
#ifndef OBS_MQTT_INTERVAL_S
#define OBS_MQTT_INTERVAL_S 60          // MQTT telemetry publish interval (_wifi builds, `set mqtt_host`)
#endif
#ifndef OBS_MQTT_CONTACTS_INTERVAL_S
#define OBS_MQTT_CONTACTS_INTERVAL_S 300 // MQTT contact-roster (name/type/coords) republish interval
#endif
#ifndef OBS_MAX_NEIGHBOURS
#define OBS_MAX_NEIGHBOURS 16           // size of the heard-neighbour table
#endif
#ifndef OBS_LOW_BATT_STRIKES
#define OBS_LOW_BATT_STRIKES 5          // consecutive low reads before shutdown (debounce)
#endif
#ifndef OBS_LOWBATT_BEACON
#define OBS_LOWBATT_BEACON 0            // 1 = broadcast a "going dark" msg on the bot
#endif                                       // channel just before low-batt shutdown (solar nodes).
#ifndef OBS_LOWBATT_BEACON_DRAIN_MS
#define OBS_LOWBATT_BEACON_DRAIN_MS 5000 // max time to wait for the beacon to actually TX
#endif                                        // before sleeping (covers flood pre-TX random delay)
// NOTE: bot_enabled / bot_channel are persisted in companion's NodePrefs
// (reliable, dedicated prefs storage) rather than here -- the DataStore blob
// store rejects sub-100-byte records and evicts by age, so it can't hold them.

// Single-cell LiPo cell voltage (mV) -> approximate state-of-charge percent
// (0..100). All observer_node targets run single-cell LiPo packs. Instantaneous
// reading, so it sags under TX load -- a rough gauge, not a coulomb-counter.
uint8_t observerLipoPercent(uint16_t mv);

struct ObserverNeighbour {
  uint8_t  prefix[4];           // first 4 bytes of pubkey
  char     name[20];
  uint32_t last_heard;          // RTC seconds
  bool     used;
};

struct ObserverStats {
  uint32_t rx_count;            // raw packets received
  uint32_t relayed;            // packets we forwarded
  uint32_t dropped;            // floods dropped by relay policy
  uint32_t boot_rtc;           // RTC time at boot
};

struct ObserverState {
  ObserverStats     stats;
  ObserverNeighbour neighbours[OBS_MAX_NEIGHBOURS];
  RateLimiter       bot_limiter{OBS_BOT_RATE_MAX, OBS_BOT_RATE_SECS};
  bool              wd_on;            // wardrive mode: beacon `!path <lat,lon>` to the control channel
  uint32_t          next_wd_ms;       // next wardrive beacon time
  uint32_t          next_advert_ms;   // when to send the next periodic advert
  uint32_t          advert_interval_s; // runtime advert cadence (`set advert_interval`, 0 = off)
  uint32_t          reboot_at_ms;     // deferred `@name reboot` deadline (0 = not armed)
  uint32_t          next_batt_ms;     // next battery sample for the `!batt` envelope/trend
  uint32_t          next_hour_ms;     // when to roll the 1h battery reference
  uint16_t          mv_min, mv_max;   // battery envelope since boot (`!batt`)
  uint16_t          mv_1h_ago;        // battery ~1h ago (boot sample until the first roll)
  char              boot_reason[14];  // why we booted, captured once at begin (`!boot`)
  float             last_rssi;        // RSSI/SNR of the most recent raw RX (for !ping)
  float             last_snr;
  uint8_t           low_batt_strikes;
  bool              wdt_on;
};
